<template>
  <div id="create">
    ...
    <div v-show="stepActive==0">
      <circle-edit-item
        :submit-data.sync="isSubmitBasic"
        :price-type-list="priceTypeList"
        @refreshClick="basicSubmit"
        @errorClick="basicError"
      />
    </div>
    <div v-show="stepActive==1" class="step-2">
      <add-member-item
        :member-item="msisdnList"
        :submit-data.sync="isSubmitMember"
        :is-trim="true"
        :is-error.sync="isMsisdnList"
        @refreshClick="memberSubmit"
        @errorClick="memberError"
      />
    </div>
...
export default {
  ...
  mounted() {
    api
      .getCirclePrice({ sa: overTime.get("saName"), type: 1 })
      .then(res => {
        const { data } = res;
  ...
  methods: {
    ...
    memberSubmit(form) {
      ...
      if (!!saName && saName.indexOf("广东") > -1) {
        api
          .shareCircle({ ...form, circleId: this.circleId })
          .then(res => {
            const { data } = res;
            if (data.code === this.$common.SUCCESS) {
              this.$notify({
        ...
        api
          .addCircleMember({ ...form, circleId: this.circleId })
          .then(res => {
            const { data } = res;
            if (data.code === this.$common.SUCCESS) {
