    ...
    <div id="list">
        <div class="top-container">
            <div class="top-box">
                ...
                <div class="text-one" :style="{'color':hasOpenBiz?'':'#FE341F'}">你的圈子视频彩铃业务{{hasOpenBiz?'已开通':'尚未开通'}}
                </div>
        ...
        <div class="list-container">
            <div class="list-box" v-for="(item,index) in ringList" :key="index" :style="{'color':item.color}">
        ...
        <logout-modal :is-modal.sync="show" :phone="infos.tel" title="开通确认" btn-text="确认开通" @refreshClick="loginOpenBusiness">
            <span>资费{{infos.price}}元/月</span>
        </logout-modal>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            openCircle() {
                ...
                if(!!saName && saName.indexOf("广东") > -1) {
                    this.$dialog.confirm({
                        ...
                        api.circleRemindOpenBiz({circleId: this.circleId, memberIds: [this.infos.tel], allFlag: 0}).then(res => {
                            const { data } = res
            ...
            circleDetail() {
                ...
                api.circleInfo({msisdn: this.infos.tel}).then(res => {
                    const { data } = res
