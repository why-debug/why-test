        ...
        <div v-else>
          ...
          >
            <van-cell style="align-items: center">
              ...
              <van-icon
                slot="right-icon"
                :style="{'visibility': (infos.latestRingName != item.name && item.status == '2' ? 'inherit' : 'hidden')}"
                name="ellipsis"
                @click="openRef(index,$event)"
              />
            </van-cell>
            <template slot="right">
              <van-button
                type="info"
                text="设置"
                @click="onSetRing(item)"
                v-if="infos.latestRingName != item.name && item.status == '2'"
              />
            </template>
          </van-swipe-cell>
...
export default {
  ...
  methods: {
    ...
    onSetRing(item) {
      this.$dialog
        ...
        .then(_ => {
          ...
          api
            .settingRing({
              circleRingId: item.circleRingId,
              circleId: this.circleId
            })
            .then(res => {
              const { data } = res;
    ...
    onRemoveMember(item) {
      this.$dialog
        ...
        .then(_ => {
          ...
          api
            .removeCircleMember([item.memberId])
            .then(res => {
              const { data } = res;
              if (data.code === this.$common.SUCCESS) {
    ...
    circleDetail() {
      api
        .circleInfo({ circleId: this.circleId, msisdn: this.msisdn })
        .then(res => {
          const { data } = res;
    ...
    addRingSubmit() {
      this.$router.push({
        name: "addRingVideo",
        query: { msisdn: this.infos.circleManagerNum }
      });
    },
    onRemindOpenBiz() {
      api
        .circleRemindOpenBiz({ circleId: this.circleId, allFlag: "1" })
        .then(res => {
          const { data } = res;
          if (data.code === this.$common.SUCCESS) {
            this.$dialog.alert({
    ...
    memberList(fn) {
      api
        .circleMemberList({ page: this.memberPage, circleId: this.circleId })
        .then(res => {
          const { data } = res;
          this.memberPage += 1;
    ...
    ringList(fn) {
      api
        .ringList({ msisdn: this.infos.circleManagerNum })
        .then(res => {
          const { data } = res;
          if (data.code === this.$common.SUCCESS) {
