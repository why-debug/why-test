<template>
    <van-popup v-model="isDialog" class="open-popup" @open="open" @close="close">
        ...
        <van-field v-model="openInfo.phone" placeholder="请输入手机号" type="tel" clearable maxlength="11"
                   :error-message="errorMessage" @clear="errorMessage=''" :error="isPhoneError"/>
        <van-field v-model="openInfo.sms" clearable placeholder="请输入短信验证码" type="number" :error="isSmsError">
        ...
        <div class="flex-row-between bottom-div">
            <div><slot/></div>
            <van-button type="info" round class="comfirm-btn"
                        :loading="loginLoading"
                        @click="submitPhone">{{btnText}}
            </van-button>
        </div>
    </van-popup>
</template>
